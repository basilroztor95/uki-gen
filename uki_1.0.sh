...
INTEL_UCODE_IMG="/boot/intel-ucode.img"
BOOSTER_LINUX_IMG="/boot/booster-linux.img"
...
main() {
    ...
    check_files_exist "$INTEL_UCODE_IMG" "$BOOSTER_LINUX_IMG" "$CMDLINE_TXT" "$LINUX_KERNEL" "$EFI_STUB"

    echo "Combining microcode and initrd images..."
    cat "$INTEL_UCODE_IMG" "$BOOSTER_LINUX_IMG" > "$COMBINED_INITRD"
