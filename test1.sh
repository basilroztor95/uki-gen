...
calculate_offsets() {
    align_hex=$(objdump -p /usr/lib/systemd/boot/efi/linuxx64.efi.stub | awk '/SectionAlignment/ {print $2}')
    align=$((16#$align_hex))

    cmdline_offs_hex=$(objdump -h "/usr/lib/systemd/boot/efi/linuxx64.efi.stub" | awk 'NF==7 {print strtonum("0x"$3) + strtonum("0x"$4)}')
